...
import xyz.alphaline.mintimetablenew.cell.TableCellView
import xyz.alphaline.mintimetablenew.cell.YxisEndView
import xyz.alphaline.mintimetablenew.cell.YxisView
import xyz.alphaline.mintimetablenew.databinding.MintableBinding
...
import xyz.alphaline.mintimetablenew.utils.getHour
...
open class BaseTimeTable : LinearLayout {
    ...
    protected var tableStartTime: Int = 8
    protected var tableEndTime: Int = 19
    ...
    protected lateinit var binding: MintableBinding
...
//        addView(v)
        binding = MintableBinding.inflate(inflater, this, true)
        ...
        if(lineColor != 0) {
            binding.mainTable.setBackgroundColor(lineColor)
            binding.topMenu.setBackgroundColor(lineColor)
            binding.leftMenu.setBackgroundColor(lineColor)
        }
    ...
    protected fun recycleTimeCell () {
        for(i in 0 until (tableEndTime - tableStartTime)) {
            ...
            if (yEndLine) {
                ...
                if (i == (tableEndTime - tableStartTime) - 1) binding.timeCell.addView(
                    YxisEndView(
                        tableContext,
                        cellHeightPx.roundToInt(),
                        leftMenuWidthPx.roundToInt(),
